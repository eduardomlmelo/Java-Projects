...
public class Jogador {
    ...
    public void randomize_player_type(){ // Definir aleatoriamente o tipo do jogador
        Random randomizer = new Random();
        int player_type = randomizer.nextInt(3); // Randomizador vai de 0 até 2 (3 - 1)
        this.player_type = player_type;
    }

    public String showPlayerType(){ // Método para mostrar o tipo do jogador, 'convertendo' o número para a palavra
        ...
        return "Erro de Tipo"; // Retorna alguma coisa só para a função não ficar com erro
    }

    public int[] playDices() throws InterruptedException{ // Método que 'joga' os dados na vez de um jogador e faz as considerações necessárias


        int[] resultado = new int[2]; // Cria o vetor que armazena a soma dos dados (posisao 0) e se houve dados iguais ou não(posicao 1)
        resultado[1] = 0; // Define por padrão que não houve valores iguais nos dados


        Random randomizer = new Random();
        int num1 = randomizer.nextInt(6) + 1;
        int num2 = randomizer.nextInt(6) + 1;
        int soma = num1 + num2;
